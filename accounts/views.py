...
import random
from .models import CustomUser, PasswordResetOTP, SignupOTP
from .utils import send_password_reset_otp_email, send_signup_otp_email
...
COLLEGE_EMAIL_DOMAIN = getattr(settings, 'COLLEGE_EMAIL_DOMAIN', 'rajalakshmi.edu.in').lower().lstrip('@')
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if not first_name or not email or not student_id or not password:
            ...
            messages.error(request, f'Please use your college email ending with @{COLLEGE_EMAIL_DOMAIN}.')
        ...
        elif CustomUser.objects.filter(student_id__iexact=student_id).exists():
            messages.error(request, 'This student ID is already registered.')
        else:
            # Keep only a hash in the temporary record; never store the raw password.
            otp = str(random.randint(100000, 999999))
            SignupOTP.objects.filter(email__iexact=email, is_verified=False).delete()
            ...
            try:
                ...
                messages.error(request, f'Unable to send verification email. Please try again. ({exc})')
...
def verify_signup_otp(request):
    """Step 2: verify the college email OTP and create the student account."""
    ...
    if request.method == 'POST':
        ...
        if not record or record.is_expired:
            ...
            if CustomUser.objects.filter(email__iexact=record.email).exists():
                messages.error(request, 'An account with this email already exists. Please log in.')
                record.delete()
                request.session.pop('signup_email', None)
                return redirect('accounts:login')

            if CustomUser.objects.filter(student_id__iexact=record.student_id).exists():
                messages.error(request, 'This student ID is already registered.')
                record.delete()
                request.session.pop('signup_email', None)
                return redirect('accounts:login')
            ...
            if CustomUser.objects.filter(username__iexact=username).exists():
                messages.error(request, 'This student ID is already associated with an account.')
                record.delete()
                request.session.pop('signup_email', None)
                return redirect('accounts:login')
            ...
            try:
                ...
                messages.error(request, 'This student information is already registered.')
                return redirect('accounts:signup')
            ...
            record.delete()
            request.session.pop('signup_email', None)
            messages.success(request, f'Account created successfully. Your username is {user.username}. Please log in.')
            return redirect('accounts:login')

    return render(request, 'accounts/verify_signup_otp.html', {'email': email})
